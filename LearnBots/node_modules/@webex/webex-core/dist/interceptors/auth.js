...
var AuthInterceptor = function (_Interceptor) {
  ...
  (0, _createClass3.default)(AuthInterceptor, [{
    ...
    value: function requiresCredentials(options) {
      ...
      if (options.uri && (options.uri.includes(this.webex.config.device.preDiscoveryServices.hydra) || options.uri.includes(this.webex.config.device.preDiscoveryServices.hydraServiceUrl))) {
        return _promise2.default.resolve(true);
      }
      ...
      return this.webex.internal.device.isSpecificService('hydra', options.uri).then(function (isHydra) {
        if (isHydra) {
          return true;
        }

        if (options.service) {
          return _this3.webex.internal.device.isService(options.service);
        }
        if (options.uri) {
          // If service Url then return or else check if whitelisted
          return _this3.webex.internal.device.isServiceUrl(options.uri).then(function (isServiceUrl) {
            if (isServiceUrl) {
              return true;
            }

            if (options.addAuthHeader === undefined && _this3.webex.config.device.validateDomains) {
              options.addAuthHeader = true;
            }

            // returns true if uri is in whitelistedServiceDomains and is requested
            if (options.addAuthHeader) {
              var matchingDomains = _this3.webex.config.device.whitelistedServiceDomains.filter(function (domain) {
